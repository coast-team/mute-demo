...
- Take a look at the linked examples on the AsciiDoc web site home
  page ***REMOVED***website***REMOVED***.  Press the 'Page Source' sidebar menu item to view
...
slidy::
  ...
  ***REMOVED***website***REMOVED***slidy.html[online].
...
- Backend plugins <<X27,take precedence>> over built-in backends with
  the same name.
- You can use the `***REMOVED***asciidoc-confdir***REMOVED***` <<X60, intrinsic attribute>> to
  refer to the built-in backend configuration file location from
  backend plugin configuration files.
- You can use the `***REMOVED***backend-confdir***REMOVED***` <<X60, intrinsic attribute>> to
...
suitable DocBook toolchain (for example, see the ***REMOVED***website***REMOVED***[AsciiDoc
website]).
...
default `sect<level>` section template). The `***REMOVED***title***REMOVED***` attribute value
is set to the value of the matched regular expression group named
'title', if there is no 'title' group `***REMOVED***title***REMOVED***` defaults to the whole
...
sets the `***REMOVED***id***REMOVED***` attribute for substitution in the subsequent block's
markup template. If a second positional argument is supplied it sets
the `***REMOVED***reftext***REMOVED***` attribute which is used to set the DocBook `xreflabel`
attribute.
...
the Python expression inside the square brackets is true. Example:
  ...
  ifeval::[***REMOVED***rs458***REMOVED***==2]
  :
  endif::[]
...
pass::
  ...
  pass:attributes,quotes[<u>the '***REMOVED***author***REMOVED***'</u>]
...
<script type="text/javascript">
  ...
  video.addEventListener('click',function()***REMOVED***
    video.play();
  ***REMOVED***,false);
</script>
++++
...
`***REMOVED***mansource***REMOVED***`, `***REMOVED***manversion***REMOVED***` and `***REMOVED***manmanual***REMOVED***` attributes
respectively. This example is from the AsciiDoc header of a man page
source file:

  :man source:   AsciiDoc
  :man version:  ***REMOVED***revnumber***REMOVED***
...
<<X31,dblatex(1)>>.  Example inline formula:
  ...
  latexmath:[$C = \alpha + \beta Y^***REMOVED***\gamma***REMOVED*** + \epsilon$]

For more examples see the ***REMOVED***website***REMOVED***[AsciiDoc website] or the
...
command-line option.  Example inline formula:
  ...
  asciimath:[`x/x=***REMOVED***(1,if x!=0),(text***REMOVED***undefined***REMOVED***,if x=0):***REMOVED***`]
...
formula:
  ...
  latexmath:[$\sum_***REMOVED***n=1***REMOVED***^\infty \frac***REMOVED***1***REMOVED******REMOVED***2^n***REMOVED***$]
...
Use the `***REMOVED***brvbar***REMOVED***` attribute reference if you need to include a | pipe
...
example:
  ...
  [strongwords]
  <strong>***REMOVED***words***REMOVED***</strong>
...
- A `[specialwords]` section entry of the form
  +name=word1***REMOVED***nbsp***REMOVED***[word2...]+ adds words to existing `name` entries.
...
replaced by text from the source element.  Use the `***REMOVED***brvbar***REMOVED***`
...
This will set an attribute reference `***REMOVED***authorinitials***REMOVED***` to the value
...
Which creates these attributes:
  ...
  ***REMOVED***author***REMOVED***, ***REMOVED***firstname***REMOVED***, ***REMOVED***lastname***REMOVED***, ***REMOVED***authorinitials***REMOVED***, ***REMOVED***email***REMOVED***,
  ***REMOVED***revdate***REMOVED***, ***REMOVED***revnumber***REMOVED***
...
- If the `<entry_name>` is omitted then the entire section is
  ...
  :xref2-inlinemacro.: <a href="#***REMOVED***1***REMOVED***">***REMOVED***2?***REMOVED***2***REMOVED******REMOVED***</a>
...
and write, here are some examples:
  ...
  Using previously defined attributes: See the ***REMOVED***1***REMOVED***[Freshmeat summary]
  or the ***REMOVED***homepage***REMOVED*** for something new ***REMOVED***new***REMOVED***. Lorem ispum ***REMOVED***footnote1***REMOVED***.
...
- Positional attributes are referred to as `***REMOVED***1***REMOVED***`,`***REMOVED***2***REMOVED***`,`***REMOVED***3***REMOVED***`,...
- Attribute `***REMOVED***0***REMOVED***` refers to the entire list (excluding the enclosing
  square brackets).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Simple attribute references take the form `***REMOVED***<name>***REMOVED***`. If the
...
`***REMOVED***<names>@<regexp>:<value1>[:<value2>]***REMOVED***`::
...
`***REMOVED***<names>$<regexp>:<value1>[:<value2>]***REMOVED***`::
        ...
        `***REMOVED***<names>$<regexp>:<value>***REMOVED***`;;
...
Attribute equality test::
  If `***REMOVED***backend***REMOVED***` is 'docbook45' or 'xhtml11' the example evaluates to
  ``DocBook 4.5 or XHTML 1.1 backend'' otherwise it evaluates to
  ``some other backend'':

  ***REMOVED***backend@docbook45|xhtml11:DocBook 4.5 or XHTML 1.1 backend:some other backend***REMOVED***

Attribute value map::
  This example maps the `frame` attribute values [`topbot`, `all`,
  `none`, `sides`] to [`hsides`, `border`, `void`, `vsides`]:

  ***REMOVED***frame@topbot:hsides***REMOVED******REMOVED***frame@all:border***REMOVED******REMOVED***frame@none:void***REMOVED******REMOVED***frame@sides:vsides***REMOVED***
...
arguments. The syntax is `***REMOVED***<action>:<arguments>***REMOVED***`.

`***REMOVED***counter:<attrname>[:<seed>]***REMOVED***`::
...
`***REMOVED***counter2:<attrname>[:<seed>]***REMOVED***`::
        Same as `counter` except the it always returns a blank string.

`***REMOVED***eval:<expression>***REMOVED***`::
...
`***REMOVED***eval3:<command>***REMOVED***`::
        Passthrough version of `***REMOVED***eval:<expression>***REMOVED***` -- the generated
        output is written directly to the output without any further
        substitutions.

`***REMOVED***include:<filename>***REMOVED***`::
...
`***REMOVED***set:<attrname>[!][:<value>]***REMOVED***`::
...
`***REMOVED***set2:<attrname>[!][:<value>]***REMOVED***`::
        Same as `set` except that the attribute scope is local to the
        template.

`***REMOVED***sys:<command>***REMOVED***`::
        Substitutes the stdout generated by the execution of the shell
        `<command>`.

`***REMOVED***sys2:<command>***REMOVED***`::
        Substitutes the stdout and stderr generated by the execution
        of the shell `<command>`.

`***REMOVED***sys3:<command>***REMOVED***`::
        Passthrough version of `***REMOVED***sys:<command>***REMOVED***` -- the generated
        output is written directly to the output without any further
        substitutions.

`***REMOVED***template:<template>***REMOVED***`::
...
predefined intrinsic attributes:
  ...
  ***REMOVED***amp***REMOVED***                 ampersand (&) character entity
  ***REMOVED***asciidoc-args***REMOVED***       used to pass inherited arguments to asciidoc filters
  ***REMOVED***asciidoc-confdir***REMOVED***    the asciidoc(1) global configuration directory
  ***REMOVED***asciidoc-dir***REMOVED***        the asciidoc(1) application directory
  ***REMOVED***asciidoc-file***REMOVED***       the full path name of the asciidoc(1) script
  ***REMOVED***asciidoc-version***REMOVED***    the version of asciidoc(1)
  ***REMOVED***author***REMOVED***              author's full name
  ***REMOVED***authored***REMOVED***            empty string '' if ***REMOVED***author***REMOVED*** or ***REMOVED***email***REMOVED*** defined,
  ***REMOVED***authorinitials***REMOVED***      author initials (from document header)
  ***REMOVED***backend-<backend>***REMOVED***   empty string ''
  ***REMOVED***<backend>-<doctype>***REMOVED*** empty string ''
  ***REMOVED***backend***REMOVED***             document backend specified by `-b` option
  ***REMOVED***backend-confdir***REMOVED***     the directory containing the <backend>.conf file
  ***REMOVED***backslash***REMOVED***           backslash character
  ***REMOVED***basebackend-<base>***REMOVED***  empty string ''
  ***REMOVED***basebackend***REMOVED***         html or docbook
  ***REMOVED***blockname***REMOVED***           current block name (note 8).
  ***REMOVED***brvbar***REMOVED***              broken vertical bar (|) character
  ***REMOVED***docdate***REMOVED***             document last modified date
  ***REMOVED***docdir***REMOVED***              document input directory name  (note 5)
  ***REMOVED***docfile***REMOVED***             document file name  (note 5)
  ***REMOVED***docname***REMOVED***             document file name without extension (note 6)
  ***REMOVED***doctime***REMOVED***             document last modified time
  ***REMOVED***doctitle***REMOVED***            document title (from document header)
  ***REMOVED***doctype-<doctype>***REMOVED***   empty string ''
  ***REMOVED***doctype***REMOVED***             document type specified by `-d` option
  ***REMOVED***email***REMOVED***               author's email address (from document header)
  ***REMOVED***empty***REMOVED***               empty string ''
  ***REMOVED***encoding***REMOVED***            specifies input and output encoding
  ***REMOVED***filetype-<fileext>***REMOVED***  empty string ''
  ***REMOVED***filetype***REMOVED***            output file name file extension
  ***REMOVED***firstname***REMOVED***           author first name (from document header)
  ***REMOVED***gt***REMOVED***                  greater than (>) character entity
  ***REMOVED***id***REMOVED***                  running block id generated by BlockId elements
  ***REMOVED***indir***REMOVED***               input file directory name (note 2,5)
  ***REMOVED***infile***REMOVED***              input file name (note 2,5)
  ***REMOVED***lastname***REMOVED***            author last name (from document header)
  ***REMOVED***ldquo***REMOVED***               Left double quote character (note 7)
  ***REMOVED***level***REMOVED***               title level 1..4 (in section titles)
  ***REMOVED***listindex***REMOVED***           the list index (1..) of the most recent list item
  ***REMOVED***localdate***REMOVED***           the current date
  ***REMOVED***localtime***REMOVED***           the current time
  ***REMOVED***lsquo***REMOVED***               Left single quote character (note 7)
  ***REMOVED***lt***REMOVED***                  less than (<) character entity
  ***REMOVED***manname***REMOVED***             manpage name (defined in NAME section)
  ***REMOVED***manpurpose***REMOVED***          manpage (defined in NAME section)
  ***REMOVED***mantitle***REMOVED***            document title minus the manpage volume number
  ***REMOVED***manvolnum***REMOVED***           manpage volume number (1..8) (from document header)
  ***REMOVED***middlename***REMOVED***          author middle name (from document header)
  ***REMOVED***nbsp***REMOVED***                non-breaking space character entity
  ***REMOVED***notitle***REMOVED***             do not display the document title
  ***REMOVED***outdir***REMOVED***              document output directory name (note 2)
  ***REMOVED***outfile***REMOVED***             output file name (note 2)
  ***REMOVED***python***REMOVED***              the full path name of the Python interpreter executable
  ***REMOVED***rdquo***REMOVED***               Right double quote character (note 7)
  ***REMOVED***reftext***REMOVED***             running block xreflabel generated by BlockId elements
  ***REMOVED***revdate***REMOVED***             document revision date (from document header)
  ***REMOVED***revnumber***REMOVED***           document revision number (from document header)
  ***REMOVED***rsquo***REMOVED***               Right single quote character (note 7)
  ***REMOVED***sectnum***REMOVED***             formatted section number (in section titles)
  ***REMOVED***sp***REMOVED***                  space character
  ***REMOVED***showcomments***REMOVED***        send comment lines to the output
  ***REMOVED***title***REMOVED***               section title (in titled elements)
  ***REMOVED***two-colons***REMOVED***          Two colon characters
  ***REMOVED***two-semicolons***REMOVED***      Two semicolon characters
  ***REMOVED***user-dir***REMOVED***            the ~/.asciidoc directory (if it exists)
  ***REMOVED***verbose***REMOVED***             defined as '' if --verbose command option specified
  ***REMOVED***wj***REMOVED***                  Word-joiner
  ***REMOVED***zwsp***REMOVED***                Zero-width space character entity

[NOTE]
======
1. Intrinsic attributes are global so avoid defining custom attributes
   with the same names.
2. `***REMOVED***outfile***REMOVED***`, `***REMOVED***outdir***REMOVED***`, `***REMOVED***infile***REMOVED***`, `***REMOVED***indir***REMOVED***` attributes are
...
5. `***REMOVED***docfile***REMOVED***` and `***REMOVED***docdir***REMOVED***` refer to root document specified on the
   asciidoc(1) command-line; `***REMOVED***infile***REMOVED***` and `***REMOVED***indir***REMOVED***` refer to the
...
6. If the input file is the standard input and the output file is not
   the standard output then `***REMOVED***docname***REMOVED***` is the output file name sans
   file extension.
7. See
   http://en.wikipedia.org/wiki/Non-English_usage_of_quotation_marks[non-English
   usage of quotation marks].
8. The `***REMOVED***blockname***REMOVED***` attribute identifies the style of the current
   block. It applies to delimited blocks, lists and tables. Here is a
   list of `***REMOVED***blockname***REMOVED***` values (does not include filters or custom
...
use the ***REMOVED***website***REMOVED***source-highlight-filter.html[source code highlighter
filter].
...
***REMOVED***website***REMOVED***index.html#_filters[AsciiDoc website].
...
|music
|A ***REMOVED***website***REMOVED***music-filter.html[music filter] is included in the
...
|source
|A ***REMOVED***website***REMOVED***source-highlight-filter.html[source code highlight filter]
is included in the distribution `./filters/` directory.

|latex
|The ***REMOVED***website***REMOVED***latex-filter.html[AsciiDoc LaTeX filter] translates
...
***REMOVED***website***REMOVED***asciidoc-graphviz-sample.html[AsciiDoc Graphviz examples].
...
***REMOVED***website***REMOVED***plugins.html[AsciiDoc website].
...
[TIP]
=====================================================================
- Assign the `gq***REMOVED***` command to the Q key with the `nnoremap Q gq***REMOVED***`
...
indent and then executing the `gq***REMOVED***` command.
...
1. Change the indentation and layout of configuration file markup
   template sections. The `***REMOVED***empty***REMOVED***` attribute is useful for outputting
...
You can view the HTML file here: ***REMOVED***website***REMOVED***article-standalone.html[]
...
space character `***REMOVED***nbsp***REMOVED***` works but is an ad hoc solution compared
...
- `***REMOVED***include:<filename>***REMOVED***` executable attribute files must reside
...
various editors listed on the ***REMOVED***website***REMOVED***[AsciiDoc website].
