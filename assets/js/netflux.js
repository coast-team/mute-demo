    ...
    if (msg.connectors.includes(WEBSOCKET)) {
      ...
      provide(WEBSOCKET).connect(msg.botUrl)
        .then(channel => {
          ...
          channel.send(msgBld.msg(NEW_CHANNEL, wc.myId, null, {
            wcId: wc.id,
            oneMsg: msg.oneMsg
          }))
...
class WebChannel {
  ...
  sendInner (serviceId, data) {
    this.manager.sendInner(this, msgBld.msg(INNER_DATA, this.myId, null, {serviceId, data}))
  }
  ...
  listen (options = {}) {
    return new Promise((resolve, reject) => {
      ...
      this.server = new WebSocketServer({
        ...
        setListenOnWebSocket(true)
        resolve()
      })

      this.server.on('error', () => {
        setListenOnWebSocket(false)
          ...
          switch (header.code) {
            case JOIN:
              ...
              if (wc === null || wc.members.length === 0) {
                if (wc === null) wc = new WebChannel({connector: WEBSOCKET})
                wc.joinAsBot(channel.channel).then(() => {
                  console.log(`Me bot ${wc.myId} + joined a wc with ${wc.members[0]}`)
                  this.onWebChannel(wc)
                })
              ...
              break
            case NEW_CHANNEL:
              console.log('Looking for WC: ' + msg.wcId + ' and know: '+ this.webChannels.length)
              this.webChannels.forEach(wc => { console.log('Know: ' + wc.id)})
              console.log(`Me bot ${wc.myId} NEW_CHANNEL received from ${header.senderId}`)
              if (wc !== null) {
