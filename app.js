...
app.get('/ajax/createDoc/:docID', function (req, res) {
	if(rooms.indexOf(req.params.docID) === -1) {
...
app.get('/createDoc', function (req, res) {
	var newDocID = 'demo';
	while(rooms.indexOf(newDocID) !== -1) {
		newDocID = createID();
	}
...
app.get('/:docID', function (req, res) {
	...
	res.setHeader('Content-Type', 'text/html');
	res.render('editor', { docID: req.params.docID, newDoc: newDoc });
});


app.get('/', function (req, res) {
	res.setHeader('Content-Type', 'text/html');
	res.render('home', { docID: 'demo' });
});
